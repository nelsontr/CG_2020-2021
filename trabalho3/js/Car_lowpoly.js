...
var light_on = true;
var holofote_1 = true, holofote_2 = true, holofote_3 = true;
var spotLightHF1, spotLightHF2, spotLightHF3;
...
var meshWheel = [];
var meshBoard = [], meshPalanque = [], meshHolofotes = [];
var meshWindow = [];
...
function createScene() {
	...
	createCars(0, 10.5, 0);
	createHolofotes(-20, 20, 0, 1);
	createHolofotes(20, 20, 0, 2);
	createHolofotes(0, 20, -20, 3);
...
function createFollowCamera() {
	...
	followcamera = new THREE.OrthographicCamera(-70, 70, 60, -20, -20, 300);
...
function onKeyDown(e) {
	...
	if (e.keyCode == 49) { // keyCode tecla 1
		if (holofote_1 == true) {
			mesh_sphere[1 - 1].material.color.setHex(0xbe935a);
			mesh_cone[1 - 1].material.color.setHex(0xbe935a);
			holofote_1 = false;

		}
		else {
			holofote_1 = true;
			mesh_sphere[1 - 1].material.color.setHex(0x00acff);
			mesh_cone[1 - 1].material.color.setHex(0x00acff);
		}
	} else if (e.keyCode == 50) { // keyCode tecla 2
		if (holofote_2 == true) {
			holofote_2 = false;
			mesh_sphere[2 - 1].material.color.setHex(0xbe935a);
			mesh_cone[2 - 1].material.color.setHex(0xbe935a);
		}
		else {
			holofote_2 = true;
			mesh_sphere[2 - 1].material.color.setHex(0x00acff);
			mesh_cone[2 - 1].material.color.setHex(0x00acff);
		}
	} else if (e.keyCode == 51) { // keyCode tecla 3
		if (holofote_3 == true) {
			holofote_3 = false;
			mesh_sphere[3 - 1].material.color.setHex(0xbe935a);
			mesh_cone[3 - 1].material.color.setHex(0xbe935a);
		}
		else {
			holofote_3 = true;
			mesh_sphere[3 - 1].material.color.setHex(0x00acff);
			mesh_cone[3 - 1].material.color.setHex(0x00acff);
		}
...
function createPalanque(x, y, z) {
	...
	meshPalanque = new THREE.Mesh(geometry, materialPalanque[1]);

	palanque.add(meshPalanque);
...
function createHolofotes(x, y, z, index) {
	...
	material_sphere = new THREE.MeshPhongMaterial({ color: 0x00acff, wireframe: false, specular: 0x00acff, shininess: 60 });
	mesh_sphere[index - 1] = new THREE.Mesh(geometry, material_sphere);
	mesh_sphere[index - 1].position.set(x, y + 2.5, z);
	holofotes.add(mesh_sphere[index - 1]);


	// raio, height, segment/
	geometry = new THREE.ConeGeometry(3, 10, 32);
	material_cone = new THREE.MeshPhongMaterial({ color: 0x00acff, wireframe: false, specular: 0xff0000, shininess: 60 });
	mesh_cone[index - 1] = new THREE.Mesh(geometry, material_cone);
	mesh_cone[index - 1].position.set(x, y + 10, z);
	holofotes.add(mesh_cone[index - 1]);
...
function addCarTopBack(obj, x, y, z) {
	...
	//luz frente
	windows.vertices.push(new THREE.Vector3(5.1, 1, 3));					//14           
	windows.vertices.push(new THREE.Vector3(5.1, 1, -3));        //15
	windows.vertices.push(new THREE.Vector3(5.1, 0, -3));        //16
	windows.vertices.push(new THREE.Vector3(5.1, 0, 3));         //17   
	windows.faces.push(new THREE.Face3(16, 15, 14));
	windows.faces.push(new THREE.Face3(17, 16, 14));

	//luz atras
	windows.vertices.push(new THREE.Vector3(-15.1, 0, 3));        //18
	windows.vertices.push(new THREE.Vector3(-15.1, 0, -3));       //19 
	windows.vertices.push(new THREE.Vector3(-15.1, -1, -3));      //20
	windows.vertices.push(new THREE.Vector3(-15.1, -1, 3));       //21
	windows.faces.push(new THREE.Face3(18, 19, 20));
	windows.faces.push(new THREE.Face3(18, 20, 21));
...
function ilumination() {
	if (!BasicFlag) {
		...
		meshPalanque.material = materialPalanque[0];
		meshWindow.material = materialWindow[0];
		mesh_lateral_car.material = material_lateral_car[0];
		...
		meshPalanque.material = materialPalanque[1];
		meshWindow.material = materialWindow[1];
		mesh_lateral_car.material = material_lateral_car[1];
...
function shadding() {
	if (!LambertFlag) {
		...
		meshPalanque.material = materialPalanque[2];
		meshWindow.material = materialWindow[2];
		mesh_lateral_car.material = material_lateral_car[2];
